...
;-
PRO eit_img_timerange_4_venustransit,dir_im=dir_im,start_date=start_date,end_date=end_date,help=help,cosmic=cosmic,gif=gif,jpg=jpg,quality_jpg=quality_jpg,no_block_fill=no_block_fill,progressive=progressive,hv_simple=hv_simple,scale=scale,quiet=quiet,latest=latest,hv_write = hv_write,hv_count = hv_count,hv_details = hv_details
...
vt_factor2 = 1.0
   ...
   for i_wave = 0,3 do begin   
		s = eit_catrd(wave = fix(hv.obs.mes[i_wave]),iday_str,/lz,nx = 1024, ny = 160)
                ; select 1024x1024 or 512x512 images only:
                fhr=(strpos(s,vt_size) gt 0)+(strpos(s,'2x(16,16)') gt 0)
                s_raw = eit_catrd(wave = fix(hv.obs.mes[i_wave]),iday_str,/lz,/raw,nx = 1024, ny = 160)
...
;	in time (until end date or current date) 
		...
		while no_files and (day_0.mjd lt min([end_date_utc.mjd,today_date_utc.mjd])) do begin
			...
			print, 'EIT_IMG_TIMERANGE: Next Image D-DATE, date_0 = ', date_0, '.'
                        s = eit_catrd(wave = fix(hv.obs.mes[i_wave]),day_0str,/lz,nx = 1024, ny = 160)
                        ; select 1024x1024 or 512x512 images only:
                        fhr=(strpos(s,'32, 5') gt 0)+(strpos(s,'2x(16,16)') gt 0)
                        s_raw = eit_catrd(wave = fix(hv.obs.mes[i_wave]),day_0str,/lz,/raw,nx = 1024, ny = 160)
                        s_raw_type=size(s_raw,/tname)
                        IF s_raw_type ne 'STRING' THEN BEGIN                         
                                ; check for "PW" and "N" tags ; what about "1"?
                           wx = where((s_raw.program eq 14 or s_raw.program eq 9) and fhr gt 0,nwx)
...
;		full_frame = fffr or ffhr
		fffr = strpos(s[i_file], '32, 5') gt 0
...
; if image is half-res, then resample to 1024x1024
                   if ffhr then begin
                      ...
                      b0 = bytscl(alog10(a(*, *) < 4*hv.eitsca.t_val[i_wave]), min = alog10(vt_factor2*ffhr_min_val[i_wave]), $
                                  max = alog10(vt_factor*4*hv.eitsca.t_val[i_wave]), top = 255b)  
                      b0=rebin(b0,1024,1024)
                   endif else begin
                      a = a > hv.eitsca.min_val[i_wave]
                      b0 = bytscl(alog10(a(*, *) < hv.eitsca.t_val[i_wave]), min =  alog10(vt_factor2*hv.eitsca.min_val[i_wave]), $
                                  max = alog10(vt_factor*hv.eitsca.t_val[i_wave]), top = 255b)                                         
                   endelse
...
;
                   if KEYWORD_SET(hv_write) then begin ; HVP
                      ...
                      b1 = bytarr(1024,1024)
                      b1[*,0:159] = b0
                      HV_EIT_IMG_TIMERANGE,h,b1,ffhr,s(i_file),this_wave,hv_details,ginfo.notgiven,sep[n_elements(sep)-1],already_written = already_written,jp2_filename = jp2_filename, venustransit = 1 ; HVP
