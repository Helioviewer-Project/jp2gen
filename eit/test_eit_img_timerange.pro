...
; create default directories; note that images in hv format are
; written in their own directory structure
...
;___from J. Ireland (2010-04-06)
; set default image type to .gif
  IF KEYWORD_SET(gif) eq 0 and KEYWORD_SET(jpg) eq 0 and KEYWORD_SET(hv_simple) eq 0 and KEYWORD_SET(hv_write) eq 0 THEN BEGIN ;*** JI, added in hv_write exception
     gif=1
...
;
;                      if not(KEYWORD_SET(hv_details)) then hv_details = 'hvs_default_eit'
;
;                      info = CALL_FUNCTION(hv_details)
;                      HV_EIT_IMG_TIMERANGE,h,b0,ffhr,s(i_file),this_wave,info
;                      hv_count = hv_count + 1
;                   endif
;___
