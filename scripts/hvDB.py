...
def UniqueNicknames(dbloc, dbName, timeStart, timeEnd):
    try:
...
def UniqueMeasurements(dbloc, dbName, timeStart, goodbad, timeEnd):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
...
def NumberFilesAll(dbloc, dbName, timeStart, timeEnd, goodbad):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
        ttt = (timeStart, timeEnd, goodbad)
...
def NumberFilesByNickname(dbloc, dbName, timeStart, timeEnd, goodbad, nickname):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
...
def NumberFilesByMeasurement(dbloc, dbName, timeStart, timeEnd, goodbad, measurement):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
...
def NumberFilesByNicknameAndMeasurement(dbloc, dbName, timeStart, timeEnd, goodbad, nickname, measurement):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
...
def MostRecentJP2(dbloc, dbName, timeEnd, daysBackMax, goodbad, nickname, measurement):
    try:
        con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        cur = con.cursor()
        ttt = (timeStart, timeEnd, goodbad, nickname, measurement)
...
def GetMeasurementsPerNickname(dbloc, dbName, timeStart, timeEnd, goodbad, nickname):
    if goodbad == -1:
        try:
            con = sqlite3.connect(dbloc + dbName, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
            cur = con.cursor()
...
def hvDailyFileAquisitionReport(dbloc,dbName,DT,monitorLoc,daysBackMax,DTmax,locationToday = 'yyyy/mm/dd/', relativeLink = '../../../', htmlFileName = 'download_stats.html', imgWidth = '600px'):
    ...
    dayNicknames =  UniqueNicknames(dbloc,dbName,dayStart,dayEnd)
    n = len(dayNicknames)
    ...
    for goodbad in range(1,-1,-1):
        ...
        for hr in range(0,24):
            ...
            data[hr] =  NumberFilesAll(dbloc, dbName, timeStart, timeEnd, goodbad)
        ...
        for j in range(0,n):
            ...
            for hr in range(0,24):
                timeStart, timeEnd = hvHourTimesForDate(date,hr)
                data[hr] =  NumberFilesByNickname(dbloc, dbName, timeStart, timeEnd, goodbad, nickname)
            ...
            #
            measurements =  GetMeasurementsPerNickname(dbloc, dbName, dayStart, dayEnd, -1, nickname)
            nm = len(measurements)
            data = np.zeros((nm,24))
            ...
            for k in range(0,nm):
                ...
                for hr in range(0,24):
                    timeStart, timeEnd = hvHourTimesForDate(date,hr)
                    data[k,hr] =  NumberFilesByNicknameAndMeasurement(dbloc, dbName, timeStart, timeEnd, goodbad, nickname, measurement)
