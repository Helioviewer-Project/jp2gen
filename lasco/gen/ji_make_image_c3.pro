...
FUNCTION JI_MAKE_IMAGE_C3, filename, FIXGAPS=fixgaps, VIDEOIMG=videoimg, PICT=pict, NOLABEL=nolabel, $
	NOLOGO=nologo, BKG=bkg, RDIFF=rdiff
;
; read the LASCO FITS fill, data and header
;
  img = lasco_readfits(filename,hdr)
  if ( (size(img,/dim))(0) eq 1024) then begin
...
;	IF (hdr.lebxsum)^2 + (hdr.sumcol)^2 GT 2 THEN img = REDUCE_STD_SIZE(img,hdr,/FULL)
     img = REDUCE_STD_SIZE(img,hdr,/FULL, /BIAS,/no_rebin) ;JAKE 030127-removed IF added /BIAS

     if (hdr.naxis1 eq 1024) then begin
        ...
        ENDIF ELSE BEGIN   
           ...
           if ( (size(imgm,/dim))(0) eq 0) then begin
              print,'JI_MAKE_IMAGE_C3: no background image; returning'
              RETURN,-1
           endif else begin
