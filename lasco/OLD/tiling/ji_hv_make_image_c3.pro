...
FUNCTION JI_HV_MAKE_IMAGE_C3, img, hdr, FIXGAPS=fixgaps, VIDEOIMG=videoimg, PICT=pict, NOLABEL=nolabel, $
  ...
  if (hdr.naxis1 eq 1024) then begin
     ...
     ENDIF ELSE BEGIN   
        ...
        if ( (size(imgm,/dim))(0) eq 0) then begin
           print,'JI_HV_MAKE_IMAGE_C3: no background image; returning'
           cimg = [-1,-1]
           RETURN,cimg
        endif else begin
