...
;-
;
PRO ji_ssc_browse_secchi_jpeg, file, name, sc, stereo_browse, hv_answer, $
                            beacon=beacon, replace=replace, $
...
;  earlier.  Don't do beacon images.  For Behind images before Feb 17, 2007,
;  only display simple images.
;
                'hi2': if (strmid(name,16,1) eq 's') and $
                  (not keyword_set(beacon)) then begin
...
;
                        ...
                        errmsg = ''
                        fxread, sccfindfits(cat[w[0]].filename), bkg, hbkg, $
                          errmsg=errmsg
                        if errmsg ne '' then begin
                            print, errmsg
                            return ;Skip if file is unreadable
...
;
                        loadct, 0, /silent
                        image = bytscl(image, min=-0.3, max=0.3)
                    endelse
                endif else return
;
...
;
        ...
        hv_answer = {temp:temp,header:header}
...
;
        ...
        if strmid(name0, 17, 1) ne '7' then begin
            ...
            if count gt 0 then file_delete, temp else $
