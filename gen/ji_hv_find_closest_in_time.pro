FUNCTION JI_HV_RETURN_OBS_TIME,file,tag_name = tag_name
...
; 
FUNCTION JI_HV_FIND_CLOSEST_IN_TIME,list,rds,tag_name = tag_name
  ...
  for i = 0,1+nint(alog(nlist)/alog(2.0)) do begin ; add a wee bit to make sure
     ...
     dlo = anytim2tai( JI_HV_RETURN_OBS_TIME(list[ilo],tag_name = tag_name ) )
     dii = anytim2tai( JI_HV_RETURN_OBS_TIME(list[iii],tag_name = tag_name ) )
     dhi = anytim2tai( JI_HV_RETURN_OBS_TIME(list[ihi],tag_name = tag_name ) )
