...
;
pro ji_make_tiles3,source_images,source_list,mission,instrument,detector,outdir,fitype,$
                  rewrite = rewrite,format = format,timestamp = timestamp,jp2 = jp2
;
;
;
progname = 'ji_make_tiles3'
;
; check input
;
...
;
   ...
   tnew = {yy:yy, mm:mm, dd:dd, hh:hh}
;
; create the appropriate directory, and rewrite files if necessary
;
   observation = {mission:mission, instrument:instrument, detector:detector, measurement:measurement}

   JI_HV_MAKE_SUBDIR, told, tnew, observation, outdir, rewrite, time_stamp = time_stamp, datatype_stamp = datatype_stamp
...
;  write the tiles
;
   sfinal = outdir(0) + time_stamp + '/' + datatype_stamp
...
;
   if keyword_set(jp2) then begin
      sfinal = outdir(1) + time_stamp + '/' + datatype_stamp
