...
; image containing the mask (alpha=0:masked,alpha=1:visible)
;
;
;-

PRO ji_write_jp2_dm,file,image,bit_rate=bit_rate,n_layers=n_layers,n_levels=n_levels,fitsheader=fitsheader,_extra=_extra,head2struct=head2struct, institute, contact,alpha=alpha,keep_tif=keep_tif,keep_xml=keep_xml,quiet=quiet

if keyword_set(alpha) then begin
   istif=query_tiff(alpha,tiff_info)
   if istif ne 1 then begin
      print,'alpha mask must be provided in TIFF format - returning.'
      return
   endif else begin
      size_im=size(image,/dim)
      if (sa[0] ne info.dimensions[0]) or (sa[1] ne info.dimensions[1]) then begin
         print,'dimensions of alpha mask do not agree with dimensions of image - returning.'
         return
      endif
   endelse
endif
...
;
; set defaults for JPEG2000 compression
;
; next line: only if compression is done inside IDL.
...
; this is how it is done inside IDL (does not support alpha channel):
;
...
; compress JP2 file by spawning kdu_compress outside of idl:
;
; write temporary TIFF file
write_tiff,file+'.tif',reverse(image_new,2),bits=8
...
; if ahlpa channel is provided, pass it on to kdu_compress to create a
; 2-component JP2 file
IF KEYWORD_SET(alpha) THEN data_in='-jp2_alpha -i '+file+'.tif'+','+alpha ELSE data_in='-i '+file+'.tif'
data_out=' -o '+file+'.jp2'
IF KEYWORD_SET(fitsheader) eq 0 THEN meta_data='' ELSE meta_data=' -jp2_box '+meta_xml

kdu_command='kdu_compress '+data_in+data_out+' Clayers='+strtrim(string(n_layers),2)+' Clevels='+strtrim(string(n_levels),2)+' -rate '+bit_rate+meta_data+quiet_flag

spawn,kdu_command

IF KEYWORD_SET(keep_tif) eq 0 THEN spawn,'rm '+file+'.tif'
