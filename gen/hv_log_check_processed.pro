...
;
FUNCTION HV_LOG_CHECK_PROCESSED,nickname
;
; Get some details on how the log file name is constructed.  Pass in a
; dummy time, in this case, UTC
;
  get_utc,utc
  c = HV_LOG_FILENAME_CONVENTION(nickname,utc2str(utc),utc2str(utc),/components)
;
; Get the log file directories
;
  log = (HV_STORAGE(nickname = nickname)).log_location
