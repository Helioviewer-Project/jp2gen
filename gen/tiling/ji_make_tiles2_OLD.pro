...
pro ji_make_tiles2,source_images,source_list,mission,instrument,detector,outdir,fitype,$
                  rewrite = rewrite,format = format,timestamp = timestamp
;
;
;
...
if (mission eq 'trac') then begin
   if (instrument eq 'TRA') then begin
      if (detector eq 'TRA') then begin
         ...
         v(0) = 0 ; no. pixels in the x direction
         v(1) = 0
         v(2) = 0 ; no. pixels in the y direction
         v(3) = 0
         v(4) = 0 ; arcsec per pixel in the x direction
         v(5) = 0
         v(6) = 0 ; arcsec per pixel in the y direction
         v(7) = 0
         v(8) = 0 ; centre position: x value
         v(9) = 0
         v(10) = 0 ; centre position: y value
         v(11) = 0
...
;
      ...
      endrep until( (float(trace_nx)/(256.0*2^(zoom_offset-zoom_base))) le 1.0)
;      zoom_offset = 13
;
...
;  calculate the zoom_offset
;
      zoom_offset = nint(alog(nx_new)/alog(2.0))
;
...
;
      offset_new_x = 4096 - nx_congrid/2
      offset_new_y = 4096 - nx_congrid/2
   endif
